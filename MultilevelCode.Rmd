...
# how many distinct schools are there? (pollev.com/sta)
...
So we are estimating: $$y_{ij} = \beta_0 + v_{j} + \epsilon_{ij} $$
...
Let's predict math scores with SES as a level I predictor still using a random intercept model, with school as the level II variable. So we are estimating: $$y_{ij} = \beta_0 + \beta_1 x_{ij} + v_{j} + \epsilon_{ij} $$
